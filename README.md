...
Subnet Masks & CIDR Notation
Subnet Mask
A subnet mask is a 32-bit number that defines the boundary between the network and host portions of an IP address. The network bits are all 1s and the host bits are all 0s.
...
Switch reads the source MAC and records it in its MAC address table
Switch looks up the destination MAC
If found → forward to that port. If not found → flood to all ports (except source)
Key characteristics:
...
How Routing Works
Device A (192.168.1.10) wants to reach Device B (192.168.2.20)
Device A checks: "Is 192.168.2.20 on my subnet (192.168.1.0/24)?" → No
Device A sends the packet to its default gateway (192.168.1.1 = router)
Router receives the packet on interface 192.168.1.1
Router looks up 192.168.2.20 in its routing table
Router finds that 192.168.2.0/24 is on another interface (192.168.2.1)
Router forwards the packet out that interface
...
PC-A sends packet to its gateway 192.168.1.1
Switch forwards via trunk to router (tagged VLAN 10)
Router receives on subinterface G0/0.10
Router routes to subinterface G0/0.30 (VLAN 30)
Router sends back to switch (tagged VLAN 30)
Switch forwards to PC-C's access port (VLAN 30)
Routing Table
...
Order matters — Most specific rules first, general rules last
Implicit deny — Most systems deny everything not explicitly permitted (Cisco default). Some systems use implicit permit (everything allowed unless denied)
Apply close to source — For deny rules, apply as close to the traffic source as possible to avoid wasting bandwidth
Be specific — Use the narrowest subnet that matches your intent
SimuLAN Tutorial Network
...
Design Decisions
VLSM — One /24 block efficiently divided; Sales gets /26 (most users), smaller departments get /27 or /28
VLANs — Isolate departments at Layer 2; broadcasts stay within each VLAN
Router-on-a-stick — Single router handles all inter-VLAN routing via subinterfaces
Trunk ports — Switch-to-router links carry all VLAN traffic tagged with 802.1Q
Access ports — End devices connect on a single VLAN, unaware of VLAN tagging
ACLs — Sales is restricted from HR and Services; Guest is fully isolated; IT has full access for administration
